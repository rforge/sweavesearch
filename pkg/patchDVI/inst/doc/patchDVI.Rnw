...
\newcommand{\patchDVI}{\textbf{patchDVI}\ }
...
for \texttt{.pdf}
output in \texttt{pdfsync} and is currently available in Synctex.  
...
the output.  (Details of the embedding are described below.)  After
...
The \verb!\Sconcordance! macro expands to a \verb!\special! macro with
prefix ``concordance:'' when
producing a \texttt{.dvi} file.  This is included verbatim in the \texttt{.dvi}
file; the \texttt{patchDVI} function scans the whole file until it finds
the record.  Source references are recorded by \LaTeX\ in \verb!\special! records; 
...
references from the \texttt{.synctex} file.  It rewrites only
the \texttt{.synctex} file when it makes its changes.


\section{Project Management Function \texttt{SweaveAll}}
...
project management function, \texttt{SweaveAll}.  This function takes a vector
of filenames, and runs Sweave on each.  After each run, it examines the
global environment for two variables:  \texttt{.SweaveFiles} and \texttt{.TexRoot}.

A code chunk in a \texttt{Rnw} file may produce a character vector
\texttt{.SweaveFiles}.  It should contain the names of \texttt{.Rnw} 
...
turn name additional \texttt{.Rnw} files to process.
...
a \texttt{.tex} file.  If it does, then that is the file that will be passed
...
\texttt{SweaveAll} is called by all of the functions desribed in the previous 
section to do step 1 of the \patchDVI steps.
...
\begin{verbatim}
 ...
 @
\end{verbatim}
Then while one is working on ``chapter.Rnw'', one can call
\texttt{SweavePDF} or \texttt{SweaveDVI} (or the MikTeX versions) 
with argument \texttt{"chapter.Rnw"} 
and the chapter will be processed through
the full sequence.  Similarly, the wrapper file (named for example
``wrapper.Rnw'') should be a \texttt{.Rnw} file that sets
\texttt{.SweaveFiles} to the complete list of files in the project.
Then one can rebuild the entire document by calling
\texttt{SweavePDF("wrapper.Rnw")}.

More complicated schemes are possible.  Each chapter can have subsections in separate
